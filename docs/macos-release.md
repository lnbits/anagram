...
6. Cleanup runs on success, errors, and handled termination signals, with an additional workflow `always()` step before upload. It restores the keychain search list and deletes the temporary keychain, P12, notarization profile, submission archive, and extracted verification copy. Credential-bearing command arguments/output are never printed. The hosted runner is also discarded by GitHub when the job ends.
